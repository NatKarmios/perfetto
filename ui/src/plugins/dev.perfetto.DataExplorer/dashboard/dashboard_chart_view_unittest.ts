...
import type * as chartRenderers from '../query_builder/charts/chart_renderers';
...
const {loaderQueries, adapters} = vi.hoisted(() => ({
  ...
  adapters: [] as chartRenderers.ChartColumnProvider[],
}));

vi.mock('../query_builder/charts/chart_renderers', async (importOriginal) => {
  const actual = await importOriginal<typeof chartRenderers>();
  return {
    ...actual,
    createChartLoaders: (_engine: unknown, query: string) => {
      loaderQueries.push(query);
    },
    renderChartByType: (ctx: {node: chartRenderers.ChartColumnProvider}) => {
